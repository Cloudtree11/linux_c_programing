...
#define ADDRSTRLEN (NI_MAXHOST + NI_MAXSERV + 10)
	 ...
	 * 服务器在尝试向一个对端已经被关闭的 socket 写入数据时收到 SIGPIPE 信号，
	 * 忽略该信号后，这种情况下 write() 会失败并返回 EPIPE 错误
	 */
	...
	/*
	 * getaddrinfo() - combines the functionality provided by the getservby and getservbypor into a single interface
	 */
    ...
    for (;;) {                  /* Handle clients iteratively */
		...
		/*
		 * getnameinfo - the inverse of getaddrinfo
		 */
